...
function testlog() {
	if [[ -n "${TESTLOG:-}" ]] ; then
		if [[ -f /.dockerenv ]]; then
			logfile="${TESTLOG}/run-docker.log"
		else
...
function preload() {
	...
	echo "Clearing uprobe tracer"
	echo 0 | sudo tee /sys/kernel/debug/tracing/tracing_on || true
	echo 0 | sudo tee /sys/kernel/debug/tracing/events/uprobes/enable || true
	echo | sudo tee /sys/kernel/debug/tracing/uprobe_events || true
	echo | sudo tee /sys/kernel/debug/tracing/trace || true
...
function prepare() {
	...
	export LD_DETECT_OUTDATED=uprobes
	export LD_DETECT_OUTDATED_DELAY=3
	export LD_DEPENDENCY_CHECK=1
	export LD_RELAX_CHECK=1
}

# local function stopping and cleaning up uprobes
function stop_uprobes() {
	if [[ -e "/sys/kernel/debug/tracing/trace" ]] ; then
		echo "stopping uprobes" | testlog
		echo 0 > /sys/kernel/debug/tracing/tracing_on
		UPROBES_SUFFIX="${TESTLOG}/uprobes-$(date +%Y-%m-%d_%H-%M-%S)"
		cat /sys/kernel/debug/tracing/uprobe_events > "${UPROBES_SUFFIX}.events" || true
		cat /sys/kernel/debug/tracing/trace | sed -e "s/^.*: \([^:]*\): (0x[0-9a-f]*)$/\1/" | egrep -v "^#" | sort | uniq -c > "${UPROBES_SUFFIX}.trace" || true
		echo "Total uprobes with $(cat "${UPROBES_SUFFIX}.events" | wc -l) events has $(cat "${UPROBES_SUFFIX}.trace" | wc -l) unique traces (restarting)" | testlog
		echo 0 > /sys/kernel/debug/tracing/events/uprobes/enable || true
...
function restart() {
	...
	$SSLBIN s_server -key $SSLDIR/key.pem -cert $SSLDIR/cert.pem -unlink -rev > "${TESTLOG}/run-${SUFFIX}-server.log" 2> "${TESTLOG}/run-${SUFFIX}-server.err" &
	PID=$!
...
function run() {
	...
	if [[ $PID -eq 0 ]] ; then
		...
		cat /sys/kernel/debug/tracing/uprobe_events > "${UPROBES_SUFFIX}.events" || true
		cat /sys/kernel/debug/tracing/trace | sed -e "s/^.*: \([^:]*\): (0x[0-9a-f]*)$/\1/" | egrep -v "^#" | sort | uniq -c > "${UPROBES_SUFFIX}.trace" || true
		echo "Current uprobes with $(cat "${UPROBES_SUFFIX}.events" | wc -l) events has $(cat "${UPROBES_SUFFIX}.trace" | wc -l) unique traces" | testlog
	fi
}
